...
Replace the example trust settings with the exact issuer, accepted audience,
and HTTPS JWKS endpoint for your tokens. Tokens must also contain the user,
tenant, and role claims described in [JWT configuration](_context/wiki/config.md#jwt-claims-validated-by-claims_layer).
Existing control-plane tokens with a different claim layout need an explicit
CEL mapping; a valid signature alone no longer grants MCP access. The production
image includes the plugin factories and
